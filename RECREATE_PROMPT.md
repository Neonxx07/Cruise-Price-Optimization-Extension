...
- APScheduler for the (mostly-stub) scheduler.
...
└── platform/
    ...
    ├── scheduler/
    │   ├── __init__.py
    │   └── jobs.py                      (APScheduler: cache cleanup + honest periodic-check stub)
    ├── config/
    │   ├── __init__.py
...
CORS wide open (`allow_origins=["*"]`) — comment it as development-only. Single module-level `BookingService()` instance shared across requests (no per-request DI). Lifespan: `setup_logging` + `init_db()` on startup; no shutdown logic beyond a stub comment (the scheduler is never started here — see §12 for why this is deliberate, not a bug).
...
### 9.11 Scheduler (`scheduler/jobs.py`)

Implement `start_scheduler()`/`stop_scheduler()` using APScheduler's `AsyncIOScheduler`, idempotent (module-global singleton). Always register a `cache_cleanup` job (`IntervalTrigger(hours=6)`) calling `CacheService.cleanup_expired()`. If `settings.scheduler_enabled`, also register a `periodic_check` job (`IntervalTrigger(minutes=settings.scheduler_interval_minutes)`) — but see §12 for exactly how honest this stub must be. Do not wire `start_scheduler`/`stop_scheduler` into the FastAPI lifespan or anywhere else — this subsystem is intentionally never actually started by the running application in this rebuild (see §12).
...
- **A real, functioning scheduler-driven periodic watchlist check.** Implement `scheduler/jobs.py`'s `_periodic_check()` as an HONEST stub — it must clearly log that it is a placeholder every time it fires (e.g. `logger.info("scheduler.periodic_check", msg="Periodic check triggered (no watchlist configured)")`) and do nothing else; do not silently no-op without logging, and do not build out real watchlist-table querying, notification sending, or result storage for it. Similarly, do not wire `start_scheduler()`/`stop_scheduler()` into the FastAPI `lifespan` or any other startup path — leave the scheduler subsystem importable-but-never-invoked, exactly as before, since it was never actually used in the original either. The cache-cleanup job inside the same scheduler module (a real 6-hourly `CacheEntry` sweep) is fine to keep fully functional — only the watchlist/periodic-price-check half is a stub.
...
- [ ] The scheduler's `_periodic_check()` stub logs an explicit "this is a placeholder" message every time it fires, and `start_scheduler`/`stop_scheduler` are never invoked from the FastAPI lifespan or anywhere else in the app.
